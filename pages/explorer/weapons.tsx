...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  for (const row of weaponData) {
    ...
    tableData.push({
      ...
      rpm_near:
        Math.round(getWeaponRpm(row.weapon_bag, row.weapon_bag.range_distance_near)*100)/100 || "-",
      rpm_mid: Math.round(getWeaponRpm(row.weapon_bag, row.weapon_bag.range_distance_mid)*100)/100 || "-",
      rpm_far: Math.round(getWeaponRpm(row.weapon_bag, row.weapon_bag.range_distance_far)*100)/100 || "-",
